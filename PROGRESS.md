...
- **Wave 9.1 hotfix**: face-grain mode was missing its Save Plan + Print buttons (copy-paste oversight from earlier waves); typing in a strip formula was clobbering after each keystroke because `refreshStripList` rewrote the input's `innerHTML` (now captures + restores focus/cursor across the rewrite). Both reported by Michael.
- **Global search (⌘K)** (queue item #5 done). New 🔍 header button + ⌘K / Ctrl+K shortcut. Searches across plans (title/type/species/pattern), projects (title/type/species/stain/finish/description), species (name/Latin/origin/uses/region/tree), supplies (name/brand/notes/source). Results grouped by category with substring-highlighted matches in copper. Up/Down/Enter for keyboard nav; Esc, backdrop click, or ⌘K again closes. Click any result to navigate — plans → Plans tab, projects → that project's detail page, species → Wood Library tab (scrolled to the card), supplies → My Shop tab (filtered to Supplies). No fuzzy matching — case-insensitive substring is enough for MVP. Mobile gets the 🔍 button (no keyboard shortcut available).
- **Hardware database** (queue item #6 done). New `Hardware` tab with 51-item pre-populated catalog: 9 glues (Titebond II/III/Original, Gorilla, CA thin/medium, epoxy 2-part / clear pour, hide glue), 12 fasteners (pocket screws, wood screws, brad/pin nails, dowels, biscuits, plugs), 14 finishes (Walrus Oil, mineral oil, Tried & True, Danish Oil, Osmo Polyx, Arm-R-Seal, wipe-on poly, shellac, tung oil, Minwax stains), 9 hardware items (drawer slides, hinges, pulls, knobs, shelf pins, magnetic catches), 7 abrasives (sandpaper grits, steel wool, sanding sponges). Filter chips by category + search + per-item editable price (override stored in `woodshop-hardware-prices-v1`, syncs across devices like Lumber Prices). Hardware also surfaces in Global Search results (searching "titebond" jumps you straight to the glue rows). Pairs with Lumber Prices for full project cost.
- **All 58 species now have tree + region data** (queue item #7 done; was 18/58 before). Added 40 entries to `WOOD_TREE_DATA` over 2 rounds: variants (Sycamore, Knotty Alder, Qtr/Rift White Oak, African Mahogany), figured maples (Curly, Birdseye, Spalted, Ambrosia), Curly Cherry, Peruvian Walnut, and 30+ exotics (Honduran Mahogany, Indian Rosewood, Gabon Ebony, Macassar Ebony, Bubinga, Bocote, Zebrawood, Bloodwood, Goncalo Alves, Osage Orange, Lignum Vitae, Katalox, Leopard Wood, Macacauba, Morado, Movingue, Patagonian Rosewood, Pau Rosa, Red Heart, Yellow Heart, Tiama, African Teak (Iroko), Amazakoue (Ovangkol), Beli, Black Limba (Korina), Canary, Ebiara, Goiabao, Ziracote). Each entry: 1-3 paragraphs of `tree` (size, leaves, bark, distinguishing features), 1-3 paragraphs of `region` (geography, climate, conservation, sourcing notes), plus a silhouette `art` key (broad/oval/narrow/conifer/cypress/spreading/birch). Wood Library cards now show "The tree" + "Where it grows" sections for every species in the catalog.
